...
export default function GreenActions() {
  ...
  const {
    ...
    greenVideos, // optional: nếu context trả về mảng link string
  } = useAppContext();

  // đảm bảo fetch chỉ gọi 1 lần
  const activitiesFetchedRef = useRef(false);

  // danh sách link đơn giản (bạn thay bằng link thật)
  const videoLinks = Array.isArray(greenVideos) && greenVideos.length
    ? greenVideos
    : [
  ...
  // try extract youtube id and return embed url
  ...
  const normalize = (url) => {
    ...
    if (/\.(mp4|webm|ogg)(\?.*)?$/i.test(s)) return { kind: 'video', src: s };
    const y = toYouTubeEmbed(s);
    if (y) return { kind: 'youtube', src: y };
    // fallback: iframe-compatible URL
    return { kind: 'iframe', src: s };
  };

  // chọn random khi mount hoặc khi videoLinks thay đổi
  useEffect(() => {
